<script>
	...
	let { ref = $bindable(null), class: className } = $props();
...
<Toggle
	...
	class={cn(
		'absolute top-1/2 right-0 size-9 min-w-0 -translate-y-1/2 p-0 hover:bg-transparent! data-[state=off]:text-grey-200 hover:data-[state=off]:text-grey-50 data-[state=on]:bg-transparent data-[state=on]:text-grey-200 hover:data-[state=on]:text-grey-50',
		{
