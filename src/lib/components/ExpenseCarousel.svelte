<script>
	import { usdFormatter, getAvatarColor, getExpenseCategoryIcon } from '$lib/utils/functions.js';
...
<Carousel.Content class="ms-0 -mr-5">
	{#each expenses as item, i (i)}
		{@const Icon = getExpenseCategoryIcon(item?.category)}
		<Command.Item
			...
			>
				<Card.Content class="px-0!">
					<a href={resolve(`${EXPENSES}#${item?.slug}`)} class="flex flex-col space-y-5 p-4 sm:p-5">
						<div
							class="flex size-12.5 items-center justify-center rounded-full text-grey-600 {getAvatarColor(
								i
							)}"
						>
							<Icon class="size-5" />
